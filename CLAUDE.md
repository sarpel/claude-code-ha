...
- **Codex Terminal Pro** (slug `codex_terminal_pro`, in `codex-terminal/`, since 2026-07) — a near-identical copy running **OpenAI's Codex CLI**. Differences: Codex installs the complete `@openai/codex` package so the CLI and companion executables stay version-matched, persists auth/sessions in `CODEX_HOME=/data/.config/codex`, supports headless login via the `openai_api_key` option (`codex login --with-api-key`), ships no armv7, and uses a global `AGENTS.md` (in `codex-config/`) instead of Claude skills for the persist-install rule. The two add-on dirs are intentionally self-contained copies — HA add-ons cannot share files — so behavioral fixes usually need applying to **both**.
