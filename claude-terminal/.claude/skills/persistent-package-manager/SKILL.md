...
This skill helps you install packages in the Claude Terminal Pro Home Assistant add-on that will **persist across container restarts and reboots**. You must NEVER use `apk add` or `pip install` directly, as those install to ephemeral storage that disappears on restart.
...
1. Go to **Settings** → **Add-ons** → **Claude Terminal Pro**
